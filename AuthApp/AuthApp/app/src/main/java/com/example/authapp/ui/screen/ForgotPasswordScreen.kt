    ...
    Column(
        ...
        OutlinedTextField(
            ...
            colors = TextFieldDefaults.colors(
                focusedContainerColor = if(isEmailValid) MaterialTheme.colorScheme.primary else Color.Red,
                unfocusedContainerColor = if (isEmailValid)MaterialTheme.colorScheme.onSurface else Color.Red,
        ...
        Button(onClick = {
            if(isEmailValid && email.isNotEmpty()){
                viewModel.resetPassword(email){ sucess ->
                    if (sucess){
                        Toast.makeText(context, "Email de recuperação enviado!", Toast.LENGTH_SHORT).show()
                        navController.navigate("login")
                    } else {
                        Toast.makeText(context, "Erro ao enviar o email.", Toast.LENGTH_SHORT).show()
                    }
                }
            } else {
